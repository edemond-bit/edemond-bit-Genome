...
int Glimmer::readGlimmerFile(NCBI *ncb,char *File_Name)
{
	FILE *fp;
	int i,j;
	RecFields rf;
	...
	if(fp != NULL)
	{
		...
		while(!feof(fp))
		{
			fscanf(fp,"%[^ ]%d\t%d\t%c%[^\n]\n",rf.name,&rf.pos1,&rf.pos2,&rf.Strand,rf.Temp_String);
			rf.Length = abs(rf.pos2 - rf.pos1) + 1;
	...
	if(fp != NULL)
	{
		...
		recNum_Plus = i;
		recNum_Minus = j;
		printf("\nTotal seq(Glimmer): %d ", recNum_Plus + recNum_Minus);
		fclose(fp);
		fp = NULL;
	}
	return 1;
...
void Glimmer::ncbi_Vs_Glimmer(NCBI *ncb, char isHypo, char *eval_FileName)
{
   FILE *fp;
   bool matched_flag;
   ...
   if((fp = fopen(eval_FileName,"a")) != NULL)
   {
	...
	{
	   	//With Hypothetical
		fprintf(fp,"%s\t%d\t%d\t%d\t%d\t%0.4lf\t%0.4lf\n","Glimmer",(abs_Count1 + abs_Count2),(stopOnly_Count1 + stopOnly_Count2),(uniq_Count1 + uniq_Count2),(missed_Count1 + missed_Count2),
	...
	else if(isHypo == 'N')
	{
		...
		fprintf(fp,"%s\t%d\t%d\t%d\t%d\t%d\t%0.4lf\t%0.4lf\t%0.4lf\n","Glimmer",(abs_Count1 + abs_Count2),(stopOnly_Count1 + stopOnly_Count2),(Our_Hypothetical_Plus + Our_Hypothetical_Minus),
